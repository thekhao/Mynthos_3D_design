...
function retarget( target, source, options = {} ) {
	...
	for ( let i = 0; i < bones.length; ++ i ) {
		...
		if ( boneTo ) {
			...
			if ( target.isObject3D && options.localOffsets && options.localOffsets[ bone.name ] ) {
	...
	for ( let frame = 0; frame < end; ++ frame ) {
		...
		for ( let j = 0; j < bones.length; ++ j ) {
			...
			if ( boneTo ) {
				...
				if ( options.hip === name ) {
					if ( ! boneData.pos ) boneData.pos = { times: new Float32Array( end ), values: new Float32Array( end * 3 ) };
				...
				if ( ! boneData.quat ) boneData.quat = { times: new Float32Array( end ), values: new Float32Array( end * 4 ) };
...
function clone( source ) {
	...
	cloned.traverse( function ( node ) {
		...
		clonedMesh.skeleton.bones = sourceBones.map( function ( bone ) { return cloneLookup.get( bone ); } );
