...
class OrbitControls extends Controls {
	...
	pan( deltaX, deltaY ) { this._pan( deltaX, deltaY ); this.update(); }
	dollyIn( dollyScale ) { this._dollyIn( dollyScale ); this.update(); }
	dollyOut( dollyScale ) { this._dollyOut( dollyScale ); this.update(); }
	rotateLeft( angle ) { this._rotateLeft( angle ); this.update(); }
	rotateUp( angle ) { this._rotateUp( angle ); this.update(); }
	...
	_handleTouchStartRotate( event ) {
		if ( this._pointers.length === 1 ) {
			...
			this._rotateStart.set( 0.5 * ( event.pageX + position.x ), 0.5 * ( event.pageY + position.y ) );
		}
	}
	...
	_handleTouchStartPan( event ) {
		if ( this._pointers.length === 1 ) {
			...
			const position = this._getSecondPointerPosition( event );
			this._panStart.set( 0.5 * ( event.pageX + position.x ), 0.5 * ( event.pageY + position.y ) );
		}
	}
	...
	_handleTouchMoveRotate( event ) {
		if ( this._pointers.length == 1 ) {
			...
			const position = this._getSecondPointerPosition( event );
			this._rotateEnd.set( 0.5 * ( event.pageX + position.x ), 0.5 * ( event.pageY + position.y ) );
		}
		this._rotateDelta.subVectors( this._rotateEnd, this._rotateStart ).multiplyScalar( this.rotateSpeed );
		const element = this.domElement;
		this._rotateLeft( _twoPI * this._rotateDelta.x / element.clientHeight );
	...
	_handleTouchMovePan( event ) {
		if ( this._pointers.length === 1 ) {
			...
			const position = this._getSecondPointerPosition( event );
			this._panEnd.set( 0.5 * ( event.pageX + position.x ), 0.5 * ( event.pageY + position.y ) );
		}
		this._panDelta.subVectors( this._panEnd, this._panStart ).multiplyScalar( this.panSpeed );
		this._pan( this._panDelta.x, this._panDelta.y );
		this._panStart.copy( this._panEnd );
	...
	_handleTouchMoveDolly( event ) {
		...
		const dx = event.pageX - position.x;
		const dy = event.pageY - position.y;
		this._dollyEnd.set( 0, Math.sqrt( dx * dx + dy * dy ) );
		this._dollyDelta.set( 0, Math.pow( this._dollyEnd.y / this._dollyStart.y, this.zoomSpeed ) );
		this._dollyOut( this._dollyDelta.y );
		this._dollyStart.copy( this._dollyEnd );
		this._updateZoomParameters( ( event.pageX + position.x ) * 0.5, ( event.pageY + position.y ) * 0.5 );
	}
	...
	_removePointer( event ) {
		...
		for ( let i = 0; i < this._pointers.length; i ++ ) {
			if ( this._pointers[ i ] == event.pointerId ) { this._pointers.splice( i, 1 ); return; }
		}
	}
	...
	_customWheelEvent( event ) {
		...
		switch ( mode ) {
			case 1: newEvent.deltaY *= 16; break;
			case 2: newEvent.deltaY *= 100; break;
		}
