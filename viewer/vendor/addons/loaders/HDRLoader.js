...
class HDRLoader extends DataTextureLoader {
	...
	parse( buffer ) {
		const rgbe_read_error = 1, rgbe_write_error = 2, rgbe_format_error = 3, rgbe_memory_error = 4;
		...
		const RGBE_VALID_PROGRAMTYPE = 1, RGBE_VALID_FORMAT = 2, RGBE_VALID_DIMENSIONS = 4;
		const NEWLINE = '\n';
		...
		const RGBE_ReadHeader = function ( buffer ) {
			const magic_token_re = /^#\?(\S+)/,
				...
				header = { valid: 0, string: '', comments: '', programtype: 'RGBE', format: '', gamma: 1.0, exposure: 1.0, width: 0, height: 0 };
			let line, match;
		...
		const RGBE_ReadPixels_RLE = function ( buffer, w, h ) {
			...
			while ( ( num_scanlines > 0 ) && ( pos < buffer.byteLength ) ) {
				...
				rgbeStart[ 0 ] = buffer[ pos ++ ]; rgbeStart[ 1 ] = buffer[ pos ++ ];
				rgbeStart[ 2 ] = buffer[ pos ++ ]; rgbeStart[ 3 ] = buffer[ pos ++ ];
				...
				for ( let i = 0; i < l; i ++ ) {
					let off = 0;
					data_rgba[ offset ] = scanline_buffer[ i + off ]; off += scanline_width;
					data_rgba[ offset + 1 ] = scanline_buffer[ i + off ]; off += scanline_width;
					data_rgba[ offset + 2 ] = scanline_buffer[ i + off ]; off += scanline_width;
		...
		const w = rgbe_header_info.width, h = rgbe_header_info.height,
		...
		switch ( this.type ) {
			case FloatType: {
				...
				data = floatArray; type = FloatType; break;
			}
			case HalfFloatType: {
				const numElements = image_rgba_data.length / 4;
				const halfArray = new Uint16Array( numElements * 4 );
				for ( let j = 0; j < numElements; j ++ ) RGBEByteToRGBHalf( image_rgba_data, j * 4, halfArray, j * 4 );
				data = halfArray; type = HalfFloatType; break;
			}
			default: throw new Error( 'THREE.HDRLoader: Unsupported type: ' + this.type );
		}
		return { width: w, height: h, data, header: rgbe_header_info.string, gamma: rgbe_header_info.gamma,
			exposure: rgbe_header_info.exposure, type, colorSpace: LinearSRGBColorSpace,
			minFilter: LinearFilter, magFilter: LinearFilter, generateMipmaps: false, flipY: true };
	}
